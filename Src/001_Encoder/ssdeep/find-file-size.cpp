...
int find_file_size(FILE *f) 
{
  int num_sectors = 0, sector_size = 0;
  int fd = fileno(f);
  struct stat sb;
...
static int
midpoint (int a, int b, int blksize)
{
  int aprime = a / blksize;
  int bprime = b / blksize;
  int c, cprime;
...
int find_dev_size(int fd, int blk_size)
{
