...
namespace core
{
	...
	{
		std::wstring strRet;
		size_t tRequiredCch = EUCKR_TO_UTF16(strInput.c_str(), strInput.length(), NULL, 0, NULL);
		strRet.resize(tRequiredCch);
		EUCKR_TO_UTF16(strInput.c_str(), strInput.length(), (WORD*)strRet.c_str(), tRequiredCch, NULL);
		return strRet;
	}
	...
	{
		std::string strRet;
		size_t tRequiredCch = UTF16_TO_EUCKR((const WORD*)strInput.c_str(), strInput.length(), NULL, 0);
		strRet.resize(tRequiredCch);
		UTF16_TO_EUCKR((const WORD*)strInput.c_str(), strInput.length(), (char*)strRet.c_str(), tRequiredCch);
		return strRet;
	}
	...
	{
		std::wstring strRet;
		size_t tRequiredCch = EUCKR_TO_UTF16(strInput.c_str(), strInput.length(), NULL, 0, ptReadSize);
		strRet.resize(tRequiredCch);
		EUCKR_TO_UTF16(strInput.c_str(), strInput.length(), (WORD*)strRet.c_str(), tRequiredCch, ptReadSize);
		return strRet;
	}
	...
	{
		std::string strRet;
		size_t tRequiredCch = UTF16_TO_EUCKR(pszInput, tInputCch, NULL, 0);
		strRet.resize(tRequiredCch);
		UTF16_TO_EUCKR(pszInput, tInputCch, (char*)strRet.c_str(), tRequiredCch);	
		return strRet;
	}
	...
	{
		std::string strRet;
		size_t tRequiredCch = UTF32_TO_EUCKR(pszInput, tInputCch, NULL, 0);
		strRet.resize(tRequiredCch);
		UTF32_TO_EUCKR(pszInput, tInputCch, (char*)strRet.c_str(), tRequiredCch);	
		return strRet;
	}
	...
	{
		std::wstring strUTF16;
		size_t tUTF16Len = EUCKR_TO_UTF16(strInput.c_str(), strInput.length(), NULL, 0, NULL);
		strUTF16.resize(tUTF16Len);
		EUCKR_TO_UTF16(strInput.c_str(), strInput.length(), (WORD*)strUTF16.c_str(), tUTF16Len, NULL);
	...
	std::vector<WORD> UTF16FromMBS(std::string strInput)
	{
		...
		size_t tRequiredCch = EUCKR_TO_UTF16(strInput.c_str(), strInput.length(), NULL, 0, NULL);
		strRet.resize(tRequiredCch);
		EUCKR_TO_UTF16(strInput.c_str(), strInput.length(), &strRet[0], tRequiredCch, NULL);
		return strRet;
	}
	...
	std::vector<DWORD> UTF32FromMBS(std::string strInput)
	{
		...
		strRet.resize(tRequiredCch);
		EUCKR_TO_UTF32(strInput.c_str(), strInput.length(), &strRet[0], tRequiredCch);
		return strRet;
	}
